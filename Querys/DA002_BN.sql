--DA002_Banner
SELECT SSBSECT_TERM_CODE AS PERIODO,
	SSBSECT_CAMP_CODE AS CAMPUS, 
	SSRATTR_ATTR_CODE AS COD_PROG, 
	STVATTR_DESC AS PROGRAMA,
	SSBSECT_SUBJ_CODE || SSBSECT_CRSE_NUMB AS COD_CURSO,
	SCBCRSE_TITLE AS NOMBRE_CURSO,
	CICLO,
	SSBSECT_CRN AS NRC,
	STVSSTS_DESC AS ESTATUS, 
	SSRXLST_XLST_GROUP AS LST_CRZ,
	SSBSECT_SCHD_CODE AS TIP_HOR, 
	SSBSECT_INSM_CODE AS MET_EDU, 
	SSBSECT_MAX_ENRL AS MAXIMO, 
	SSBSECT_ENRL AS REAL, 
	SSBSECT_SEATS_AVAIL AS RESTANTE,
	SCBCRSE_BILL_HR_LOW AS HRS_CREDITO,
	SPRIDEN_ID AS ID_DOCENTE,
	DOCENTE_NAME AS DOCENTE,
	SIRASGN_PRIMARY_IND AS ID_PRINC, 
	STVWKLD_DESC AS TIPO_JORNADA,
	STVFCST_DESC AS ESTADO_DOCENTE,
	SSRMEET_START_DATE AS FECHA_INICIO, 
	SSRMEET_END_DATE AS FECHA_FIN, 
	SSRMEET_MON_DAY AS LUNES, 
	SSRMEET_TUE_DAY AS MARTES,
	SSRMEET_WED_DAY AS MIERCOLES,
	SSRMEET_THU_DAY AS JUEVES,
	SSRMEET_FRI_DAY AS VIERNES,
	SSRMEET_SAT_DAY AS SABADO,
	SSRMEET_SUN_DAY AS DOMINGO, 
	CASE WHEN SSRMEET_BEGIN_TIME IS NOT NULL THEN SSRMEET_BEGIN_TIME ||' - '|| SSRMEET_END_TIME END AS HORA ,
	SSRMEET_BLDG_CODE AS EDIFICIO,
	SSRMEET_ROOM_CODE AS AULA,
	SSRRPRG_PROGRAM_IND AS INCLUIR_EXCLUIR_PROG,
	SSRRPRG_PROGRAM AS COD_PROGRAMAS_COMPARTIDOS, 
	SMRPRLE_PROGRAM_DESC AS PROGRAMAS_COMPARTIDOS,
	SSRRCMP_CAMP_IND AS INCLUIR_EXCLUIR_CAMP, 
	SSRRCMP_CAMP_CODE AS CAMPUS_COMPARTIDOS, 
	SSBSECT_PREREQ_CHK_METHOD_CDE AS TIPO_PRERREQUISITO, 
	REQUISITOS,
	BLOQUES_HORARIOS
FROM (SELECT DISTINCT
      ...
      FROM SSBSECT
			...
			LEFT JOIN STVSSTS ON SSBSECT_SSTS_CODE = STVSSTS_CODE
			...
			LEFT JOIN (SELECT SIRASGN_TERM_CODE, SIRASGN_CRN, SIRASGN_PIDM, SIRASGN_CATEGORY, SIRASGN_PRIMARY_IND
						...
						FROM SIRASGN
								LEFT JOIN SIBINST ON SIRASGN_PIDM = SIBINST_PIDM AND SIRASGN_TERM_CODE >= SIBINST_TERM_CODE_EFF
			...
			LEFT JOIN SPRIDEN ON SIRASGN_PIDM = SPRIDEN_PIDM AND SPRIDEN_CHANGE_IND IS NULL
			LEFT JOIN STVFCST ON SIBINST_FCST_CODE = STVFCST_CODE
			LEFT JOIN STVWKLD ON SIBINST_WKLD_CODE = STVWKLD_CODE
			LEFT JOIN SSRATTR ON SSBSECT_TERM_CODE = SSRATTR_TERM_CODE AND SSBSECT_CRN = SSRATTR_CRN
			LEFT JOIN STVATTR ON SSRATTR_ATTR_CODE = STVATTR_CODE
			LEFT JOIN (SELECT SSRRPRG_TERM_CODE AS SSRRPRG_TERM_CODE_1, SSRRPRG_CRN AS SSRRPRG_CRN_1, SSRRPRG_PROGRAM_IND
						FROM SSRRPRG WHERE SSRRPRG_REC_TYPE = 1
			...
			LEFT JOIN (SELECT SSRRCMP_TERM_CODE AS SSRRCMP_TERM_CODE_2, SSRRCMP_CRN AS SSRRCMP_CRN_2, SSRRCMP_CAMP_CODE
						FROM SSRRCMP WHERE SSRRCMP_REC_TYPE = 2
		...
		LEFT JOIN (SELECT DISTINCT smrdorq_SUBJ_CODE, smrdorq_CRSE_NUMB_LOW, SUBSTR(smrdorq_AREA,1,4) AS MAJOR, smrdorq_TERM_CODE_EFF
							, MAX(smrdorq_TERM_CODE_EFF) OVER(PARTITION BY smrdorq_SUBJ_CODE,smrdorq_CRSE_NUMB_LOW,SUBSTR(smrdorq_AREA,1,4)) AS MAX_TERM
							, CASE WHEN SUBSTR(smrdorq_AREA,LENGTH(smrdorq_AREA)-1) IN ('01','02','03','04','05','06','07','08','09','10','11','12')
								THEN TO_NUMBER(SUBSTR(smrdorq_AREA,LENGTH(smrdorq_AREA)-1)) END AS CICLO
						FROM smrdorq
					  ) LOE_AREA_COURSE ON SSBSECT_SUBJ_CODE = smrdorq_SUBJ_CODE AND SSBSECT_CRSE_NUMB = smrdorq_CRSE_NUMB_LOW
                                        AND SSRATTR_ATTR_CODE = MAJOR AND smrdorq_TERM_CODE_EFF = MAX_TERM
	WHERE SSBSECT_TERM_CODE IN ('220435') --periodo
